...
class MyTerminalPost extends TerminalPost {
	view(vnode) {
		...
		var showAvatar =
			(mode === 'all-replies' && lastPost) || mode === 'always' || (mode === 'non-op-replies' && discussion.lastPostedUser() != discussion.user());

		return (
			<span>
				{lastPost ? icon('fas fa-reply') : ''} {showAvatar ? avatar(user, { className: 'ComposerBody-lastPostAvatar' }) : ''}
...
app.initializers.add('rob006/flarum-ext-last-post-avatar', () => {
	...
	override(DiscussionListItem.prototype, 'view', function (vnode) {
		...
		if (app.forum.attribute('lastPostAvatarMode') !== 'replace-main' || this.showFirstPost() || !discussion.replyCount()) {
			return content;
		}
